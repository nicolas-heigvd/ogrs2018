...
globe. The image is then superimposed with the 3D model giving the impression that
...
To tackle this issue, we developped a semi-automatic process to georeference images.
The users will be shown only images having a maximum number of neighbor images.
These neighbor images are the ones with which they share some overlay. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Online image databases are drastically growing (several hundreds of million 
new photos every day) with the advent of sensors becoming more and more accessible. 
The smapshot database is also brought to grow, in a more modest way but sufficiently 
so that the georeferencing of the images currently carried out by volunteer 
Internet users will no longer be enough anymore.

In order to considerably simplify their task, this work shows that it is possible 
...
However, they require a training phase that is time consumming and need 
significant computational resources.

In recent years, deep learning methods such as convolutional neural networks 
have shown that they are also very well suited to classifying or searching images 
in very large databases \citep{weyand2016}.

But there is one more step to achieve to geolocate images. 
Some studies have shown that it is possible to geolocate large quantities of 
images only based on their comparison with a database of millions of geotagged 
images \citep{hays2015,weyand2016}.
%Some results are sometimes surprising.

This work brings a novel contribution in geolocating images for the smapshot
database. It also proposes a way to insert this approach in a user friendly way on
the existing plateform.
...
Therefore their position and orientation can be used; 1) as a ground truth to 
validate and evaluate the geolocation computation and 2) to build sets of images 
having high chances to show some overlay. To build such subsets we have run 
the DBSCAN clustering algorithm \citep{ester1996} on the database.
...
image pair, we can remove some outliers in this first matching results.
 

\subsubsection*{Geometrical filter}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Finally, in order to eliminate the last outliers, we developed a geometrical filter.
...
\section*{Discussion}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Results that are close to reality are often obtained when the camera was closed
to the capture portion of the landscape (fig.\,\ref{ex1}).
...
\section*{Conclusion}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Results showed that the pose computation of a query image is quite efficient
when some conditons are respected, especially the accuracy of the reference image
pose which influences the DEM relative position.
...
Furthermore, as the matching process is a O(n$^{2}$) complex problem, 
it would take too much time to tackle any new large images dataset with 
no known a priori positions where clusters cannot be built. 
This is still a challenge that would require a machine learning approach or 
some heavier computation steps.
